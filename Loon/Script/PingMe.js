/*
@Name：PingMe 自动化签到+视频奖励 (Loon版 - 修复版)
@Author：怎么肥事 
  ...
  if (!raw) {
    ...
    function fetchApi(path) {
      return new Promise((resolve, reject) => {
        ...
        if (isLoon) {
          // Loon: $httpClient.get(options, callback(error, response, data))
          // 注意：成功时 error 为 null，data 是响应体字符串
          $httpClient.get(options, (error, response, data) => {
            if (error) {
              console.log(`【${scriptName}】请求失败: ${error}`);
              reject({ error: error });
            } else {
              console.log(`【${scriptName}】${path} 响应状态: ${response.status}`);
              // 关键修复：Loon 的 data 直接是 body 字符串，需要包装成 {body: data} 格式以兼容后续代码
              resolve({ body: data });
            }
          });
        } else if (isQuanX) {
          // QX: $task.fetch(options).then(response => {response.body})
          ...
          setTimeout(() => {
            ...
            fetchApi('videoBonus').then(res => {
              try {
                ...
                msgs.push(`❌ 视频${i}：解析失败`);
                console.log(`【${scriptName}】视频${i}解析错误: ${e}, 原始数据: ${res.body}`);
                resolve();
              }
            }).catch(err => {
    ...
    fetchApi('queryBalanceAndBonus').then(res => {
      try {
        ...
        msgs.push('❌ 查询：解析失败');
        console.log(`【${scriptName}】查询余额解析错误: ${e}, 原始数据: ${res.body}`);
      }
      ...
      try {
        ...
        msgs.push('❌ 签到：解析失败');
        console.log(`【${scriptName}】签到解析错误: ${e}, 原始数据: ${res.body}`);
      }
